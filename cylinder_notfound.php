...
<div class="jumbotron jumbotron-header rounded-0 bg-secondary text-white">
    <div class="row">
        <div class="col-6">
            <h1 class="display-4">MISSING CYLINDER</h1>
        </div>
        <div class="col-6 text-right">
            <button class="btn btn-primary" id="form_btn" style="font-weight: 600; width: 150px;">ADD CYLINDER</button><br>
            <button class="btn btn-secondary mt-2" style="font-weight: 600; width: 150px;" onclick="found()">CYLINDER</button>
        </div>
        <!-- สถิติ -->
...
<div class="wrapper-form shadow">
    <div id="dataGridNotFound" class="mt-2"></div>
</div>
...
<script>
    ...
    function get_data() {
        $.ajax({
            ...
            success: function (data) {
                ...
                dataArrNotFound = dataArr.filter(function(d){
                    return d.fieldData.not_found == "Not Found";
                });
                create_table_not_found(dataArrNotFound)
            },
            error: function (error) {
                console.log(error);
                get_token_startup();
    ...
    function fetch_summary(data) {
        console.log(data);
        $('#notfound_val').html(data[0].fieldData.not_found.toLocaleString());      
    }
    ...
    function create_table_not_found(data) {
        const dataGridNF = $("#dataGridNotFound").dxDataGrid({
            dataSource: data,
            ...
            onCellPrepared: function (e) {
                ...
                if (e.rowType === "data" && e.column.dataField === "fieldData.final_status") {
                    e.cellElement.css("background-color", e.data.fieldData.final_status == "ACCEPTED" ? "#00AF50" : e.data.fieldData.final_status == "HT OVERDUE" ? "#FFBF00" : e.data.fieldData.final_status == "HT EXPIRING" ? "#FCE8D8" : e.data.fieldData.final_status == "REJECTED" ? "#FF0000" : "");
                }
        ...
        // $('.dx-toolbar-after').prepend('<button type="button" class="btn btn-outline-dark" onclick="found()" style="line-height: 1.3; border-color: #ddd;">CYLINDER</button>');
    };

    function found() {
        window.location.href = 'https://aims-muda.cpoc.com.my/CPOC_AIMS/index.php?module=cylinder';
    }
